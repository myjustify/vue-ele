(function(root, factory) {
  ...
  this.BMAP_DRAWING_MARKER = 'marker' // 鼠标画点模式
  this.BMAP_DRAWING_POLYLINE = 'polyline' // 鼠标画线模式
  this.BMAP_DRAWING_CIRCLE = 'circle' // 鼠标画圆模式
  this.BMAP_DRAWING_RECTANGLE = 'rectangle' // 鼠标画矩形模式
  this.BMAP_DRAWING_POLYGON = 'polygon' // 鼠标画多边形模式
  ...
  DrawingManager.prototype._bindCircle = function() {
    ...
    var endAction = function(e) {
      ...
      !Screenshot.prototype.initialize && initScreenshot()
      ...
      !Operate.prototype.initialize && initOperate()
      ...
      moveMarker.addEventListener('dragging', function(e) {
        ...
        var isright = e.latLng.lng > centerPoint.lng
        var halfLeftPoint = new BMap.Point(halflng, centerPoint.lat)
  ...
  DrawingManager.prototype._bindPolylineOrPolygon = function() {
    ...
    var dblclickAction = function(e) {
      ...
      // 裁剪
      try {
        if (me._enableGpc && window.gpcas && me._drawingType === 'polygon') {
      ...
      var limit = null
      if (me.limit) {
        limit = me._drawingType === 'polygon' ? me.limit.area : me.limit.distance
      }
  ...
  function drawcircle(center, radius) {
    ...
    var lat1 = (Math.PI / 180) * cy
    var lng1 = (Math.PI / 180) * cx
    ...
    for (var i = 0; i < 271; i += 9) {
      ...
      var point = new BMap.Point(x * (180 / Math.PI), y * (180 / Math.PI))
      points.push(point)
    }
  ...
  function getPointByDistance(srcPoint, distance, direction) {
    ...
    var dlng = Math.atan2(Math.sin(tc) * Math.sin(d) * Math.cos(lat1), Math.cos(d) - Math.sin(lat1) * Math.sin(y))
    var x = ((lng1 - dlng + Math.PI) % (2 * Math.PI)) - Math.PI
    var point = new BMap.Point(tmplng || x * (180 / Math.PI), tmplat || y * (180 / Math.PI))
